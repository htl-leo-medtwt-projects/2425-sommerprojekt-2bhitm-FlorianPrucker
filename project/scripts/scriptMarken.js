...
document.addEventListener('click', (ev) => {
	...
	if (ev.target.id !== 'container' && ev.target.id !== 'sidebar' && ev.target.id !== 'sidebarButton' && ev.target.id !== 'boxes' && ev.target.id !== 'search') {
		const clickedBox = ev.target.closest(".element");
		if (clickedBox || ev.target.id === 'inh3' || ev.target.id === 'inimg') {
			toggle++
		}
		if (toggle % 2 != 0) {
			container.style.display = 'block'
			container.classList.add('markenBoxAnim1')
			setTimeout(() => {
				container.classList.remove('markenBoxAnim1')
			}, 2000);
			toggle++
		}
		else {
			container.classList.add('markenBoxAnim2')
			setTimeout(() => {
				container.classList.remove('markenBoxAnim2')
			}, 2000);
